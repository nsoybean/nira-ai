...
export default function ChatPage() {
  ...
  const [actualConversationId, setActualConversationId] = useState<string | null>(isNewChat ? null : conversationId);
  const { theme, setTheme } = useTheme();

  const { messages, setMessages, status, error, sendMessage, stop, regenerate } = useChat({
    id: actualConversationId || undefined, // Pass conversation ID only if it exists
    experimental_throttle: 50, // to make streaming smoother
    transport: new DefaultChatTransport({
      api: "/api/chat",
      // only send the last message to the server:
      prepareSendMessagesRequest({ messages, id }) {
        return { body: { message: messages[messages.length - 1], id: actualConversationId } };
      },
    }),
    async onFinish() {
      // If this was the first message in a new chat, get the conversation ID
      if (isNewChat && !actualConversationId) {
        // Fetch the latest conversation (which was just created)
        try {
          ...
          if (conversations.length > 0) {
            ...
            // Reload conversations list to show the new chat
  ...
  // Load messages from database on mount (skip for new chats)
  useEffect(() => {
  ...
  const handleNewChat = () => {
    // Just navigate to /chat/new - no API call needed
